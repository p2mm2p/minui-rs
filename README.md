...
11. [平台实现 (platforms/)](#11-平台实现-platforms)
   ...
   - 11.3 [settings：文件通信替代共享内存](#113-settings--文件通信替代共享内存)
   - 11.4 [平台列表](#114-平台列表)
   - 11.5 [启动链：从开机到进入 MinUI](#115-启动链从开机到进入-minui)
...
```
minui-rs/
├── Cargo.toml              ← workspace（20 个 member）
│
├── crates/                 ← Rust 库和二进制
│   ├── common/             ← 共享基础（types, utils, paths）
│   ├── platform/           ← Platform trait 定义
│   ├── render/             ← 软件渲染器
│   ├── power/              ← 电源管理
│   ├── minui/              ← 启动器二进制
│   └── minarch/            ← 模拟器前端二进制
...
│   ├── rg35xxplus/         ← 完整参考实现
│   ├── pc/                 ← PC 桌面平台（minifb，开发测试用）
│   ├── tg5040/             ← 🟡 开发中（常量就绪，IO 待实现）
│   └── ...（其余 10 个平台）
│
...
- `platform` 定义 Platform trait，被 minui、minarch 和各平台实现依赖
- `render` 和 `power` 被两个二进制共同需要
...
| `workspace/<平台>/libmsettings/` | `platforms/<平台>/src/settings.rs` | 设置管理 |
...
主循环维护两个不同的时间量：**帧预算**和**`dt_ms`（delta time）**。它们是两个独立的概念，服务于不同的目的。
...
// ... poll + power + navigation + render ...
let elapsed = frame_start.elapsed();
let elapsed_ms = elapsed.as_millis() as u32;
if elapsed_ms < 16 {
    std::thread::sleep(Duration::from_millis((16 - elapsed_ms) as u64));
}
```

对应 C 原版 `GFX_sync()` 中的 `SDL_Delay(FRAME_BUDGET - frame_duration)`。帧预算保证每帧大约 16ms，画面 60fps。

#### dt_ms（delta time）

用于计时器——休眠倒计时、自动关机、电源状态更新：

```rust
let dt_ms = last_frame.elapsed().as_millis().min(100) as u32;
let pwr_dirty = power.update(dt_ms);
// ... check_autosleep(dt_ms) ...
```

对应 C 原版 `PWR_update()` 内部读取 `now - last_update`。

#### 为什么不能把 dt_ms 硬编码为 16

帧预算是**目标**，不是**保证**。真实场景中帧时间可能远大于 16ms：

| 场景 | 实际帧时间 | 硬编码 16 的后果 | `last_frame.elapsed()` 的行为 |
|------|-----------|-----------------|------------------------------|
| 正常帧 | 16ms | 正常 | 返回 ~16ms |
| I/O 阻塞（SD 卡慢） | 50ms | **丢失 34ms**——休眠计时器不推进 | 返回 ~50ms |
| HDMI 热插拔 | 2000ms | **丢失 1984ms**——系统完全不知道时间流逝 | `.min(100)` → 返回 100ms |
| OS 调度抖动 | 17ms | 正常 | 返回 ~17ms |

`.min(100)` 的上限截断是防御性的：如果某帧真的卡了几秒（比如存储 I/O 阻塞），不会让计时器一次性跳进几秒——那可能导致直接关机而不是休眠。每次最多推进 100ms，系统有时间感知"异常"。
...
```c
// minui.c 主循环
GFX_startFrame();             // frame_start = SDL_GetTicks()
unsigned long now = SDL_GetTicks();
PAD_poll();
PWR_update(&dirty, ...);      // 内部自己算 delta = now - last_update
// ...
GFX_sync();                   // 帧预算：如果超了 FRAME_BUDGET 就不等
// GFX_sync 内部：frame_duration = SDL_GetTicks() - frame_start
//             if (frame_duration < FRAME_BUDGET) SDL_Delay(FRAME_BUDGET - frame_duration)
```

`GFX_startFrame()` + `GFX_sync()` 管理帧预算（画面节奏），`PWR_update()` 内部管理 delta time（计时器）。两个时间量在 C 中同样独立。
...
┌─ Frame N 开始 ────────────────────────────────────────────┐
│                                                          │
│  frame_start = Instant::now()          ← 记录帧起点       │
│  dt_ms = last_frame.elapsed()          ← 距上一帧起点的实际经过时间 │
│  poll_input(&mut pad)                  ← 读取按键         │
│  power.update(dt_ms)                   ← 推进休眠/关机计时器 │
│  check_autosleep(dt_ms)               ← 空闲满 30s？→ 休眠│
│  navigation / render / flip           ← 画面更新          │
│                                                          │
│  elapsed = frame_start.elapsed()       ← 本帧工作耗时     │
│  if elapsed < 16ms: sleep(16 - elapsed) ← 补齐到帧预算    │
│                                                          │
│  last_frame = frame_start              ← 作为下一帧的起点 │
│                                                          │
├─ Frame N+1 开始 ─────────────────────────────────────────┤
│  dt_ms = last_frame.elapsed()          ← ≈ 16ms（含本帧工作和睡眠）│
│  ...                                                     │
└──────────────────────────────────────────────────────────┘
```
...
**对应原版**：`platform.h`（常量 #define）+ `api.h`（方法签名）。Rust 实现在 `crates/platform/src/platform.rs`。

### 8.1 Platform trait — 支持 20+ 设备的秘诀
...
**对应原版**：`api.c` 中 GFX 部分。Rust 实现在 `crates/render/src/lib.rs`。使用 [fontdue](https://crates.io/crates/fontdue) 进行字体光栅化——纯 Rust 实现，无需 FreeType/SDL_ttf。
...
**对应原版**：`api.c` 中 PWR 部分。Rust 实现在 `crates/power/src/lib.rs`。

### 10.1 PowerManager — 电源管理状态机
...
### 11.3 settings — 文件通信替代共享内存

MinUI 的三个进程（keymon、minui、minarch）需要共享亮度、音量、HDMI 状态等信息。
...
ftruncate(shm_fd, sizeof(Settings));
settings = mmap(NULL, ..., MAP_SHARED, shm_fd, 0);
settings->brightness = 5;  // 直接写共享内存

// minui 进程（client，后启动）
shm_fd = shm_open(SHM_KEY, O_RDWR, 0644);
settings = mmap(NULL, ..., MAP_SHARED, shm_fd, 0);
int b = settings->brightness;  // 直接读共享内存，瞬时可见
```
...
┌───────────────────────────────────────────────────┐
│              同一块物理内存                         │
│  struct Settings { int brightness; ... }          │
│  mmap(MAP_SHARED) → 所有进程直接读写               │
└───────────────────────────────────────────────────┘
```

#### Rust 方案：/tmp/settings/ 文件
...
| | C 共享内存 | Rust 文件 |
|----|----------|----------|
| 数据格式 | 二进制 struct | 纯文本字符串 |
| 粒度 | 整个 struct 一把读写 | 每个值独立文件 |
| 可见性 | `mmap` 后立即对所有进程可见 | `read()` 时获取最新值 |
| 持久化 | 需手动 `write()` 到 msettings.bin | 已在文件中，不额外操作 |
| 调试 | 需解析二进制结构体 | `cat /tmp/settings/brightness` |
| 崩溃安全 | `shm_unlink` 可能泄漏 | OS 自动回收 tmpfs |
| unsafe | `mmap` 裸指针 | 零 unsafe |
| 同步机制 | 依赖 ARM 对齐 int 写入的原子性（无显式屏障） | 内核 VFS 层序列化（有保证） |

#### 文件通信如何避免数据竞争

关键前提：**只有 keymon 一个进程写入。** 这是单写者多读者模式（SWMR），不存在并发写冲突。

在此前提下，内核保证了读取的原子性：

1. **每个值只有一个数字**（如 `"7"`），远小于 4KB 页大小。Linux 的 `write()` 系统调用对小于 `PIPE_BUF`（4096 字节）的数据保证原子性——不会被其他读写交错。

2. **每个读写操作都是一次系统调用完成**。`fs::write()` 调 `write()`，`fs::read_to_string()` 调 `open()`+`read()`+`close()`。一个 `read()` 要么看到旧内容（写入前），要么看到新内容（写入后），永远不会看到半更新的中间状态——因为 1 字节的内容一次系统调用就写完了。

3. **每个进程有自己的文件描述符**。不存在共享文件指针（seek position）问题。每次读写时 VFS 层都从磁盘/inode 获取最新内容。

4. **对比 C 共享内存的风险**：C 版依赖 "ARM 平台上对齐的 `int` 写是原子的" 这一假设，没有显式内存屏障（如 `__sync_synchronize()` 或 C11 atomics）。如果 Settings 结构体扩展或移植到不保证原子性的平台，可能读取到部分更新的值。文件方案把这个责任交给内核 VFS 层——内核已经过数十年验证。

#### 负责这些逻辑的 Rust 文件

```
写入（仅 keymon 进程）：
platforms/<name>/src/keymon.rs     ← 唯一决策者：读 /dev/input/event1 按键
    │                                   MENU+VOL± → settings.set_brightness()
    │                                   单按VOL± → settings.set_volume()
    ▼
platforms/<name>/src/settings.rs   ← 工具库：提供文件读写 API
    │                                   Settings::set_brightness() → fs::write()
    ▼
/tmp/settings/brightness           ← 内核 VFS 保证原子写入

读取（minui 进程，初始化时）：
crates/power/src/lib.rs            ← PowerManager::new()
    └─ read_setting_from_tmpfs() → fs::read_to_string("/tmp/settings/brightness")

读取（minui 进程，运行时）：
crates/minui/src/state.rs          ← 每帧从 power.brightness / power.volume 渲染状态栏
```

| 文件 | 角色 |
|------|------|
| `platforms/<name>/src/keymon.rs` | **唯一写入者**：按键事件 → 调用 settings API 写入文件 |
| `platforms/<name>/src/settings.rs` | 文件读写工具库：提供 `set_brightness()`/`set_volume()`/`brightness()`/`volume()` 等 API，封装 `fs::write`/`fs::read` |
| `crates/power/src/lib.rs` | **读取方**：`PowerManager::new()` 从 `/tmp/settings/` 读取初始值；内存中的副本通过 `adjust_brightness()` 维护 |
| `crates/minui/src/state.rs` | **使用方**：每帧从 `power.brightness` / `power.volume` 渲染硬件状态栏 |

#### 完整数据流：从按键到界面刷新

以用户调节亮度为例（MENU + VOL+），全过程涉及 5 个文件、2 个进程、2 种 IPC 机制：
...
│    │                                                          │
│    ▼                                                          │
│  Linux kernel: /dev/input/event1 产生 evdev 事件               │
│    │                                                          │
│    ▼                                                          │
│  keymon.rs: poll loop                                         │
│    │                                                          │
│    ├─ libc::read(input_fd, &ev, sizeof(ev))                  │
│    │  ev.code == PLUS(115), ev.value == 1 → up_just_pressed   │
│    │                                                          │
│    ├─ 处理输入状态：                                          │
│    │  menu_pressed && up_just_pressed                         │
│    │    │                                                     │
│    │    ▼                                                     │
│    │  let val = settings.brightness();  // 读取当前值          │
│    │  if val < 10 {                                          │
│    │      settings.set_brightness(val + 1);                   │
│    │  }                                                       │
│    │    │                                                     │
│    │    ▼                                                     │
│    │  settings.rs:                                            │
│    │    write_u8("brightness", 6)                             │
│    │      → fs::write("/tmp/settings/brightness", "6")      │
│    │        → write() 系统调用                                │
│    │          → 内核 VFS 原子写入 tmpfs                       │
│    │                                                          │
└────┼──────────────────────────────────────────────────────────┘
     │
     │  /tmp/settings/brightness 内容变为 "6"
     │
┌────┼────── minui 进程 ────────────────────────────────────────┐
│    │                                                          │
│    │  【首次启动时】                                           │
│    ▼                                                          │
│  PowerManager::new()                                          │
│    ├─ read_setting_from_tmpfs("brightness", 5)               │
│    │    → fs::read_to_string("/tmp/settings/brightness")     │
│    │    → Ok("6") → parse → 6                                │
│    └─ self.brightness = 6                                     │
│                                                               │
│    │  【运行时：用户在当前 minui 会话中按键】                  │
│    ▼                                                          │
│  state.rs: MinUi::run() 主循环                                │
│    ├─ platform.poll_input(&mut pad)  // 读硬件按键             │
│    ├─ power.update(dt_ms)                                     │
│    ├─ handle_setting_input()                                  │
│    │    │                                                     │
│    │    ▼                                                     │
│    │  power.handle_setting_input(                             │
│    │      &pad,                                               │
│    │      Button::MENU,           // mod_brightness            │
│    │      Button::NONE,           // mod_volume (没按)         │
│    │      Button::PLUS,           // mod_plus                  │
│    │      Button::MINUS,          // mod_minus                 │
│    │  )                                                        │
│    │    │                                                     │
│    │    ▼                                                     │
│    │  power::adjust_brightness(1)                             │
│    │    ├─ self.brightness = (6 + 1).clamp(0, 10) → 7        │
│    │    ├─ self.show_setting = 1    // 显示亮度条             │
│    │    └─ self.setting_display_timer = 2000  // 持续 2 秒    │
│    │                                                          │
│    ├─ [重绘]                                                  │
│    │  HardwareStatus {                                        │
│    │      brightness: power.brightness,  // = 7               │
│    │      volume: power.volume,                               │
│    │      show_setting: 1,  // 显示中                         │
│    │      ...                                                 │
│    │  }                                                       │
...
│    │    ├─ 清屏（fill 黑色）                                   │
│    │    ├─ 渲染游戏列表                                        │
│    │    ├─ 渲染硬件状态栏（亮度条 progress=7/10）              │
│    │    └─ flip() → 屏幕显示                                   │
│    │                                                          │
│    └─ [2 秒后] power.setting_display_timer → 0               │
│         → show_setting = 0 → 亮度条消失                       │
│                                                               │
└───────────────────────────────────────────────────────────────┘
```
...
1. **keymon 和 minui 各自独立处理按键**。keymon 处理后台按键（无论当前是否在 minui 界面），minui 处理前台按键（仅在 minui 界面时）。两个进程不互相通信——它们通过 `/tmp/settings/` 共享最终状态。

2. **两套写入路径，一套读取路径**：
   - `keymon.rs → settings.rs → fs::write()` — 后台写入
   - `state.rs → PowerManager::adjust_brightness()` — 前台写入（内存）
   - `PowerManager::new() → read_setting_from_tmpfs()` — 读取（仅在启动时）

3. **time-of-check-to-time-of-use (TOCTOU) 不是问题**：因为 keymon 运行时 minui 通常不在前台（用户正在玩游戏），反之亦然。两个写入路径不会同时活跃。
...
### 11.5 启动链：从开机到进入 MinUI

MinUI 的启动是一个三层接力过程。理解这个设计的关键前提是：**嵌入式设备的固件无法被 zip 包修改**，必须借助设备原版系统的"应用文件夹"机制或手动替换系统文件，在设备闪存中植入拦截器。

#### 三层架构

```
┌──────────────────────────────────────────────────────────────┐
│  设备内部闪存 (NAND / eMMC / TF1)                            │
│  ┌────────────────────────────────────────────────────────┐  │
│  │  BOOT 拦截器                                          │  │
│  │  · 在首次安装时由 MainUI / install.sh 写入闪存         │  │
│  │  · 每次开机被固件调用（如 /usr/trimui/bin/runtrimui）  │  │
│  │  · 只做一件事：检查 SD 卡上是否有 updater              │  │
│  │  · 有 → 跳转 updater    无 → 回退原版系统             │  │
│  └────────────────────────────────────────────────────────┘  │
└──────────────────────────┬───────────────────────────────────┘
                           │ exec
┌──────────────────────────▼───────────────────────────────────┐
│  SD 卡 — .tmp_update/（安装后自动删除）                      │
│  ┌────────────────────────────────────────────────────────┐  │
│  │  updater（通用）→ boot.sh（平台专属）                  │  │
│  │  · updater 读 /proc/cpuinfo 识别平台                   │  │
│  │  · 跳转到对应 boot.sh（= platforms/<name>/install/）   │  │
│  │  · boot.sh 检测 MinUI.zip → 显示进度 → unzip → 启动   │  │
│  └────────────────────────────────────────────────────────┘  │
└──────────────────────────┬───────────────────────────────────┘
                           │ exec
┌──────────────────────────▼───────────────────────────────────┐
│  SD 卡 — .system/<platform>/（永久留存）                     │
│  ┌────────────────────────────────────────────────────────┐  │
│  │  launch.sh → minui → minarch                           │
│  │  · 设置环境变量、启动 keymon、配置 GPIO                │
│  │  · 循环运行 minui，通过 /tmp/next 文件与 minarch 通信  │
│  └────────────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────────┘
```
...
| 层 | 存在位置 | 写入时机 | 职责 |
|----|---------|---------|------|
| BOOT 拦截器 | 设备闪存（非 SD 卡） | 首次安装时由安装脚本写入 | 每次开机即运行，不依赖 SD 卡 |
| updater + boot.sh | SD 卡 `.tmp_update/` | zip 解压前存在，安装后自删 | 平台检测、解压、安装进度画面 |
| SYSTEM/ | SD 卡 `.system/` | zip 解压时写入，永久留存 | 实际运行时文件 |

核心原因：设备**开机时 SD 卡可能还没挂载好**，所以第一层必须存在设备闪存上；但 unzip 工具、安装图片、平台检测逻辑体积大且变化频繁，放在 SD 卡更适合。

#### 统一的 updater：一份脚本服务十二个平台

`skeleton/BOOT/common/updater` 是唯一一段"跨平台共享"的启动代码。通过 `/proc/cpuinfo` 识别设备：

```sh
case $(cat /proc/cpuinfo) in
    *"sun8i"*)    PLATFORM="trimuismart" ;;  # 也可能是 my282
    *"SStar"*)    PLATFORM="miyoomini"   ;;
    *"TG5040"*)   PLATFORM="tg5040"      ;;
    *"0xd03"*)    PLATFORM="zero28"      ;;
    *"0xd05"*)    PLATFORM="my355"       ;;
    ...
esac
exec "/mnt/SDCARD/.tmp_update/$PLATFORM.sh"
```

`$PLATFORM.sh` 即 `platforms/<name>/install/boot.sh`，负责平台专属逻辑。

#### 两种 BOOT 劫持模式

根据设备是否支持"应用文件夹自动发现"，植入拦截器的方式分两种：

**模式 A：应用文件夹自动发现**

原版固件扫描 SD 卡特定目录并执行 `MainUI` 或 `init.sh`。用户只需复制 BOOT 文件夹到 SD 卡，**无需手动刷写闪存**——脚本内部已包含闪存写入逻辑。

适用：trimui、miyoo、magicx 系列。

**模式 B：手动替换系统文件**

用户必须**手动**复制文件到设备闪存/TF1 的指定位置。开机时设备执行该文件，检测 SD 卡上的 updater。

适用：rg35xx（替换 dmenu.bin → /misc）、rg35xxplus（替换 TF1 dmenu.bin）、gkdpixel（手动执行 install.sh）、m17（复制 em_ui.sh）、rgb30（刷 Moss 固件）。

#### 各平台启动链详解

##### 1. Trimui Smart Pro / Brick（tg5040）— 模式 A

用户复制 `trimui` 文件夹 + `MinUI.zip` 到 SD 卡根目录，开机。

```
[首次开机]
原版固件扫描 SD 卡 → 发现 trimui/app/MainUI
  │
  ├─ MainUI（skeleton/BOOT/trimui/app/MainUI）：
  │   · mv /usr/trimui/bin/runtrimui.sh → runtrimui-original.sh  备份原版
  │   · cp ./runtrimui.sh → /usr/trimui/bin/runtrimui.sh         写入拦截器到闪存
  │   · cp .tmp_update → /mnt/SDCARD/                            复制 updater 到 SD 卡
  │   · rm -rf trimui/                              删除自身，防止下次再触发
  │   · exec updater
  │
  ├─ updater → 识别 TG5040 → exec .tmp_update/tg5040.sh
  │
  ├─ boot.sh（platforms/tg5040/install/boot.sh）：
  │   · 检测 /mnt/SDCARD/MinUI.zip
  │   · 检测 Brick：strings /usr/trimui/bin/MainUI | grep "Trimui Brick"
  │   · show.elf ./brick/updating.png（Brick 有独立图片目录）
  │   · ./unzip -o MinUI.zip -d /mnt/SDCARD
  │   · rm MinUI.zip
  │   · install.sh（tg5040 无此文件，跳过）
  │   · 首次安装（.system/ 刚创建）→ reboot
  │   · 更新（.system/ 已存在）→ exec launch.sh
  │
  └─ launch.sh → GPIO + keymon & + while minui_exec; do minui; done

[后续开机]
固件调用 runtrimui.sh（闪存上的拦截器）
  → .tmp_update/updater 不存在 → exec runtrimui-original.sh → 回退原版
```

**tg5040 特色**：Brick 有独立的安装图片目录 `./brick/`；`update.sh` 会检查并更新闪存中的 `runtrimui.sh`（从 `.system/tg5040/dat/runtrimui.sh`）；同时负责旧 Brick 平台名 `tg3040` → `tg5040` 的配置迁移。

##### 2. Trimui Smart（trimuismart）— 模式 A

与 tg5040 一致但替换目标是 `/etc/main`：
...
main.sh（每次开机）：
  · 等待 SDCARD 挂载 → 有 updater → 执行
  · 无 updater → /etc/main.original（回退原版）

boot.sh（platforms/trimuismart/install/boot.sh）：
  · ./leds_off（关灯）→ show.elf → ./unzip → exec launch.sh
  · launch.sh 由 while 循环包裹，退出后 poweroff
```

##### 3. Miyoo Mini / Mini Plus（miyoomini）— 模式 A

用户复制 `miyoo`（Mini Plus 需改名为 `miyoo354`）文件夹 + `MinUI.zip` 到 SD 卡。

```
[首次开机]
原版固件扫描 SD 卡 → 发现 miyoo/app/MainUI
  │
  ├─ MainUI（skeleton/BOOT/miyoo/app/MainUI）：
  │   · cat /proc/cpuinfo → "SStar" → ./miyoomini.sh
  │
  ├─ miyoomini.sh（skeleton/BOOT/miyoo/app/miyoomini.sh）：
  │   · 检测 Plus：/customer/app/axp_test 存在
  │   · (仅 Miyoo Mini) 通过 fw_setenv 修复老旧 bootcmd
  │   · cp .tmp_update → /mnt/SDCARD/
  │   · rm -rf miyoo354/（删除自身）
  │   · exec updater
  │
  ├─ updater → PLATFORM="miyoomini" → exec .tmp_update/miyoomini.sh
  │
  ├─ boot.sh（platforms/miyoomini/install/boot.sh）：
  │   · 初始化 LCD 背光：echo 0 > pwmchip0/export → period → duty_cycle → enable
  │   · cat /proc/ls; sleep 1; LCD_INIT=1（LCD 硬件初始化）
  │   · show.elf ./installing.png 或 ./updating.png
  │   · mv .tmp_update .tmp_update-old → unzip（系统 unzip，非 ./unzip）
  │   · rm MinUI.zip; rm -rf .tmp_update-old
  │   · 调用 install.sh（= .system/miyoomini/bin/install.sh，但这是空操作！见下方说明）
  │   · while [ -f launch.sh ]; do launch.sh; done（循环重启 MinUI）
  │   · reboot
```

**Miyoo 的 install.sh 和 update.sh 均为空操作**：`platforms/miyoomini/install/update.sh` 内容只有一行 `exit 0`。这在 zip 解压后会被放在 `.system/miyoomini/bin/install.sh`。boot.sh 虽然调用它，但实际不执行任何操作——因为 Miyoo 在 BOOT 阶段（`miyoomini.sh`）已经完成了所有闪存写入。

注意：Miyoo 和 **my282、my355、zero28** 的 update.sh 都是同样的 `exit 0` 空操作。

##### 4. Miyoo A30（my282）/ Miyoo Flip（my355）— 模式 A

与 Miyoo Mini 共用同一个 `MainUI`（`skeleton/BOOT/miyoo/app/MainUI`），通过 `/proc/cpuinfo` 分发：

- `"sun8i"` → `my282.sh`（A30）
- `"0xd05"` → `my355.sh`（Flip）

各自的 boot.sh 与 miyoomini 基本一致，细微区别：

| 平台 | unzip 方式 | CPU governor 路径 | 退出方式 | 特殊操作 |
|------|-----------|------------------|---------|---------|
| miyoomini | 系统 `unzip` | `cpu0/scaling_governor` | `reboot` | LCD 初始化 |
| my282 | `./unzip` | `cpu0/scaling_governor` | `poweroff` | — |
| my355 | 系统 `unzip` | `policy0/scaling_governor` | `poweroff` | 清 framebuffer（`cat /dev/zero > /dev/fb0`） |

my355 的 update.sh 注释写的是 `becomes .system/my282/bin/install.h`——这是一个复制粘贴错误，但实际文件内容同样是 `exit 0`。

##### 5. MagicX XU Mini M（magicmini）— 模式 A

用户复制 `magicx` 文件夹 + `MinUI.zip` 到 TF2（右槽）卡。需要先在 TF1（左槽）刷写 Moss 固件。

```
[首次开机]
Moss 固件（TF1）→ 扫描 TF2 → 发现 magicx/init.sh
  │
  ├─ init.sh（skeleton/BOOT/magicx/init.sh）：
  │   · cp .tmp_update → TF2 卡
  │   · rm -rf magicx/
  │   · exec updater
  │
  ├─ updater → PLATFORM="magicmini" → exec boot.sh
  │
  ├─ boot.sh（platforms/magicmini/install/boot.sh）：
  │   · 进度画面：直接 dd 写 BMP 到 /dev/fb0（不用 show.elf）
  │   · 系统 unzip
  │   · 调用 install.sh（如果存在）
  │   · 调用 launch.sh（注意路径：$SYSTEM_PATH/paks/MinUI.pak/launch.sh，无 $PLATFORM 子目录）
  │   · sync && shutdown now（不循环，单次执行后关机）
```

**MagicX 特殊点**：launch.sh 路径是 `$SYSTEM_PATH/paks/` 而非 `$SYSTEM_PATH/$PLATFORM/paks/`——这是唯一不使用平台子目录的平台。

##### 6. MagicX Mini Zero 28（zero28）— 模式 A

与 magicmini 共用 `magicx/init.sh`，通过 updater 的 `/proc/cpuinfo` 检测（`"0xd03"`）。

boot.sh 与 miyoo 家族更接近：使用 `show.elf`、`mv .tmp_update .tmp_update-old` 安全备份、`while` 循环 launch.sh、退出后 `poweroff`。

##### 7. RG35XX 原版（rg35xx）— 模式 B

用户**手动**复制 `dmenu.bin` 到 TF1 卡的 **MISC 分区**根目录，复制 `MinUI.zip` 到 TF2 卡根目录。

```
[开机]
原版固件从 /misc 加载 dmenu.bin（已被替换为 MinUI 版本）
  → 检查 TF2 上的 .tmp_update/updater → 存在 → 执行
  │
  ├─ updater → PLATFORM="rg35xx" → exec .tmp_update/rg35xx.sh
  │
  └─ boot.sh（platforms/rg35xx/install/boot.sh）：
      · unzip MinUI.zip → SD 卡
      · exec launch.sh
```

**install.sh 是 rg35xx 最关键的部分**——直接刷写设备的 `/misc` 分区：

```sh
mount -o remount,rw /dev/block/actb /misc
cp dat/dmenu.bin /misc        # 启动菜单（每次更新都刷）
cp dat/ramdisk.img /misc       # 内核 ramdisk（每次更新都刷）
# 以下仅首次安装时刷写：
cp dat/boot_logo.bmp.gz /misc  # 开机画面
cp dat/charging.png /misc      # 充电画面
touch /misc/.minstalled        # 安装标记
```

更新时通过 MD5 比较 `/misc/` 文件与 `dat/` 中版本，只刷写变化的部分（`boot_logo` 和 `charging.png` 不更新，保留用户可修改的版本）。

##### 8. RG35XX Plus / H / SP / 40XX 等（rg35xxplus）— 模式 B

用户**手动**复制 `dmenu.bin` 到 TF1 卡 "NO NAME" 分区（FAT32 含 `anbernic` 文件夹），`MinUI.zip` 到 TF2 卡。

流程与 rg35xx 类似，但 install.sh 更简单——只需更新 TF1 上的 dmenu.bin：

```sh
cp $SYSTEM_PATH/dat/dmenu.bin /mnt/mmc   # 更新 TF1 的启动菜单
```

不需要刷 `/misc` 分区。同时负责旧平台名迁移：`rg40xxcube` → `rg35xxplus`（复制配置并加 `-cube` 后缀）。

##### 9. GKD Pixel（gkdpixel）— 模式 A（手动触发）

用户复制 `gkdpixel` 文件夹 + `MinUI.zip` 到 SD 卡 "ROMS" 分区。**与原版系统交互**：

```
开机 → 原版系统 → APP 文件夹 → 打开 "file manager"
  → 导航到 /media/roms/gkdpixel/
  → 选中 install.sh → "Execute"

install.sh（platforms/gkdpixel/install/install.sh）：
  · cp boot.sh → /usr/sbin/frontend_start   写入闪存
  · exec updater

后续开机：
  frontend_start（已被替换）→ 检查 .tmp_update/updater → 执行
```

gkdpixel 没有 boot.sh——拦截器 `frontend_start` 直接负责启动链。

##### 10. M17（m17）— 模式 B

用户复制 `em_ui.sh` + `MinUI.zip` 到 SD 卡根目录。

```
开机 → 执行 /sdcard/em_ui.sh
  → 检查 .tmp_update/updater → 执行

install.sh（platforms/m17/install/install.sh）：
  · tar xf dat/extra-libs.tar -C .system/m17/lib/
  · cp dat/em_ui.sh → /sdcard/
```

没有 boot.sh——`em_ui.sh` 直接充当拦截器。

##### 11. RGB30 — 模式 B

使用 Moss 固件替换左槽 SD 卡的系统。Moss 内建了 updater 检测，无需 `skeleton/BOOT/` 下任何文件。无 boot.sh、无 install.sh。

##### 12. Powkiddy M17 / RGB30 的共性

这两个平台的启动完全依赖外部固件（Moss），MinUI 本身只提供 SD 卡上的运行时文件。

#### install.sh、update.sh、boot.sh 的分工

以下按"是否有 boot.sh"重新分类：

**有 boot.sh（7 个平台）**：tg5040, trimuismart, miyoomini, my282, my355, magicmini, zero28

boot.sh 的统一模式：
1. `echo performance > cpu governor`（提升安装速度）
2. 检测 `MinUI.zip` 是否存在
3. 显示进度画面（`show.elf` 或 `dd` 写 BMP 到 /dev/fb0）
4. `unzip` 或 `./unzip` 解压
5. `rm MinUI.zip`
6. 调用 install.sh（如果存在）
7. 首次安装 → reboot（或 exec launch.sh 然后 loop）

但各平台在细节上有差异：

| 平台 | 进度画面 | unzip | .tmp_update 清理 | launch.sh 调用 | 退出 |
|------|---------|-------|-----------------|---------------|------|
| tg5040 | `show.elf ./$DEVICE/$ACTION.png` | `./unzip` | 覆盖 | `exec`（替换进程） | launch.sh 决定 |
| trimuismart | `./leds_off; show.elf` | `./unzip` | 覆盖 | `while` 循环 | `poweroff` |
| miyoomini | `show.elf`（先初始化 LCD） | 系统 `unzip` | `mv`→`unzip`→`rm -rf` | `while` 循环 | `reboot` |
| my282 | `show.elf` | `./unzip` | `mv`→`unzip`→`rm -rf` | `while` 循环 | `poweroff` |
| my355 | `show.elf`（先清 fb0） | 系统 `unzip` | `mv`→`unzip`→`rm -rf` | `while` 循环 | `poweroff` |
| zero28 | `show.elf` | 系统 `unzip` | `mv`→`unzip`→`rm -rf` | `while` 循环 | `poweroff` |
| magicmini | `dd` 写 BMP | 系统 `unzip` | 覆盖 | 单次调用 | `sync && shutdown now` |

**无 boot.sh（5 个平台）**：rg35xx, rg35xxplus, gkdpixel, m17, rgb30

这些平台由 dmenu.bin、frontend_start、em_ui.sh、Moss 等二进制/外部固件直接处理启动链。它们只有 install.sh（用于首次安装时写入闪存）：

| 平台 | install.sh 做什么 |
|------|------------------|
| rg35xx | 刷 `/misc` 分区：dmenu.bin + ramdisk.img + boot_logo + charging.png |
| rg35xxplus | 更新 TF1 dmenu.bin + 迁移 rg40xxcube 配置 |
| gkdpixel | 复制 frontend_start 到 `/usr/sbin/` |
| m17 | 解压 .so 库 + 复制 em_ui.sh 到 SD 卡根 |
| rgb30 | 无 install.sh（Moss 固件处理一切） |

**update.sh 的角色**（7 个平台有）：

| 平台 | update.sh 作用 |
|------|--------------|
| tg5040 | **实际的 install.sh**：更新闪存 runtrimui.sh + 迁移 Brick 旧配置 |
| miyoomini | `exit 0`（空操作，闪存写入已在 BOOT 阶段完成） |
| my282 | `exit 0` |
| my355 | `exit 0`（注释错误写成了 `install.h`） |
| zero28 | `exit 0` |
| magicmini | 清理旧 DTB 文件 |

**注意 tg5040 的特殊性**：它的 `update.sh` 并非空操作——它会更新闪存中的 `runtrimui.sh`，同时处理旧名称 `tg3040` → `tg5040` 的配置迁移。这个脚本在每次 boot.sh 之后执行，等同于其他平台的 install.sh 角色。
...
┌─ 首次安装 ───────────────────────────────────────────────────┐
│                                                              │
│ 用户：放 trimui/ + MinUI.zip 到 SD 卡，插入开机               │
│                                                              │
│ 开机                                                         │
│ ├── 原版固件扫描 SD 卡 → 发现 trimui/app/MainUI              │
│ ├── MainUI 执行（来自 skeleton/BOOT/trimui/app/）：           │
│ │   ├── 备份 runtrimui.sh → runtrimui-original.sh            │
│ │   ├── 写入 runtrimui.sh → /usr/trimui/bin/（闪存，永久）    │
│ │   ├── 复制 .tmp_update/ → /mnt/SDCARD/                     │
│ │   ├── 删除 trimui/（防止下次再触发）                        │
│ │   └── exec updater                                         │
│ ├── updater（来自 skeleton/BOOT/common/）：                   │
│ │   └── cat /proc/cpuinfo → "TG5040" → exec tg5040.sh        │
│ ├── boot.sh（来自 platforms/tg5040/install/）：               │
│ │   ├── mount SD 卡 rw（Brick 需要）                          │
│ │   ├── CPU governor → userspace, 锁定 2GHz                  │
│ │   ├── 检测 MinUI.zip 存在                                   │
│ │   ├── 检测 Brick：strings MainUI | grep "Trimui Brick"     │
│ │   ├── show.elf ./brick/updating.png（Smart Pro 无 brick 目录）│
│ │   ├── ./unzip -o MinUI.zip -d /mnt/SDCARD                  │
│ │   ├── rm MinUI.zip                                         │
│ │   ├── install.sh（跳过：tg5040 无此文件）                   │
│ │   ├── 首次安装（.system/ 刚创建）→ reboot                   │
│ │   └── 更新（.system/ 已存在）→ exec launch.sh              │
│ └── launch.sh（来自 skeleton/SYSTEM/tg5040/paks/MinUI.pak/）：│
│     ├── GPIO 初始化（电机、肩键灯、LED、DIP 开关）            │
│     ├── CPU governor → userspace                             │
│     ├── tinymix 音频路由                                     │
│     ├── usb_device.sh                                        │
│     ├── killall wpa_supplicant/udhcpc（禁用 WiFi）            │
│     ├── keymon &（按键监控）                                  │
│     ├── trimui_inputd &（原版输入守护进程）                   │
│     └── while [ -f /tmp/minui_exec ]; do minui; done         │
│         exit → 检测 /tmp/next → eval → 启动 minarch          │
│         循环结束 → sync → shutdown                           │
└──────────────────────────────────────────────────────────────┘
...
│ ├── 固件调用 runtrimui.sh（闪存上的拦截器）                   │
│ │   ├── 等待 SD 卡挂载（最多 5 秒）                           │
│ │   ├── .tmp_update/updater 不存在                            │
│ │   └── exec runtrimui-original.sh → 回退原版系统             │
│ │                                                              │
│ 如需更新 MinUI：                                              │
│   放 trimui/MainUI + MinUI.zip → SD 卡 → 开机                │
│   MainUI 再次触发 → boot.sh 检测已存在的 .system/ → 更新模式  │
│   → 不 reboot，直接 exec launch.sh                           │
│                                                              │
│ 如需恢复原版：                                                │
│   删除 .tmp_update/updater 和 .system/                       │
│   固件调用 runtrimui.sh → 无 updater → 回退原版               │
└──────────────────────────────────────────────────────────────┘
```
...
| `api.h/c`（GFX） | `crates/render/` |
| `api.h/c`（PWR） | `crates/power/` |
...
| C 函数 | Rust 函数 | 所属 |
|--------|----------|------|
| `GFX_blitPill` | `fb_draw_pill` | render |
| `GFX_blitBattery` | `fb_draw_battery` | render |
| `GFX_blitText` | `fb_draw_text` | render |
| `GFX_blitMessage` | `draw_message` | render |
| `GFX_truncateText` | `truncate_text` | render |
| `PWR_init` / `PWR_update` | `PowerManager::new()` / `update()` | power |
| `PWR_preventAutosleep` | `prevent_autosleep` | power |
...
- **settings 文件化**：用 `/tmp/` 文件替代共享内存，无 unsafe、可调试
...
```
minui-rs/
├── Cargo.toml              ← workspace（20 members）
├── README.md               ← 完整报告（本文档）
├── resources/              ← BPreplayBold-unhinted.otf
│
├── crates/
│   ├── common/src/         ← types.rs, utils.rs, paths.rs
│   ├── platform/src/       ← platform.rs, paths.rs
│   ├── render/src/         ← lib.rs (UiRenderer + 字体 + 原语)
│   ├── power/src/          ← lib.rs (PowerManager)
│   ├── minui/src/          ← state.rs, scan.rs, launch.rs, main.rs
│   └── minarch/src/        ← core/game/config/input/video/audio/save/menu/main_loop.rs
│
├── platforms/
│   ├── pc/src/             ← ✅ platform.rs (完整 Platform impl, minifb 桌面参考实现)
│   ├── tg5040/src/         ← 🟡 lib.rs (常量就绪, IO待实现), keymon.rs, settings.rs
...
| `platform` | ✅ 完成 | Platform trait + Framebuffer + TestPlatform |
| `render` | ✅ 完成 | RGB565 软件渲染器 + fontdue 字体 |
| `power` | ✅ 完成 | PowerManager 状态机 |
| `minui` (启动器) | ✅ 完成 | 扫描、导航、启动逻辑全部就绪 |
| `minarch` (模拟器前端) | 🟡 骨架就绪 | trait 和模块结构完成，核心逻辑待实现 |
| `pc` 平台 | ✅ 完成 | minifb 桌面参考实现 |
