...
```sh
cargo build -p minput --features platform-tg5040/smart --release
```

每个 feature 对应一个 `platform-*` crate。编译时只有一个平台实现被链接进来。minput 的 `Cargo.toml` 声明 optional 平台依赖 + `platform-tg5040` feature（`["dep:platform-tg5040"]`），与 `crates/clock/Cargo.toml` 完全同模式。
